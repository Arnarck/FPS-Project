...
public class PlayerInventory : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < inventory.Length; i++) // Creates the inventory, and disable all slots
        {
            ...
            inventory[i].slot_ui = Instantiate(slot_prefab).GetComponent<SlotData>();
            inventory[i].slot_ui.slot_index = i;
    ...
    public bool store_item(Item item)
    {
        ...
        for (int i = 0; i < inventory.Length; i++)
        {
            if (inventory[i].is_avaliable && inventory[i].type.Equals(ItemType.NONE)) // Tries to find an empty and avaliable inventory slot
            {
                ...
                inventory[i].slot_ui.count_text.text = null; // Don't display item count for single items ("single" means 1 item per slot)
    ...
    public void remove_item(int i)
    {
        ...
        inventory[i].slot_ui.count_text.text = null;
        inventory[i].slot_ui.button.image.sprite = null;
    }
