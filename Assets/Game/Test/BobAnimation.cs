...
public class BobAnimation : MonoBehaviour
{
    ...
    Quadrant sin_quadrant_on_key_released, cos_quadrant_on_key_released, a_sin_quadrant_on_key_released;
    ...
    [Header("Sin(angular)")]
    public float angular_sin_multiplier = 1f;
    ...
    private void Start()
    {
        ...
        sin_quadrant_on_key_released = cos_quadrant_on_key_released = a_sin_quadrant_on_key_released = Quadrant.FIRST;
    }
    ...
    void Update()
    {
        ...
        {
            sin_quadrant_on_key_released = get_quadrant_of_current_wave(sin_time);
            cos_quadrant_on_key_released = get_quadrant_of_current_wave(cos_time);
            a_sin_quadrant_on_key_released = get_quadrant_of_current_wave(a_sin_time);

            if (sin_quadrant_on_key_released == Quadrant.THIRD) sin_time = update_third_quadrant_to_fourth(sin_time);
            else if (sin_quadrant_on_key_released == Quadrant.SECOND) sin_time = update_second_quadrant_to_first(sin_time);

            if (cos_quadrant_on_key_released == Quadrant.THIRD) cos_time = update_third_quadrant_to_fourth(cos_time);
            else if (cos_quadrant_on_key_released == Quadrant.SECOND) cos_time = update_second_quadrant_to_first(cos_time);

            if (a_sin_quadrant_on_key_released == Quadrant.THIRD) a_sin_time = update_third_quadrant_to_fourth(a_sin_time);
            else if (a_sin_quadrant_on_key_released == Quadrant.SECOND) a_sin_time = update_second_quadrant_to_first(a_sin_time);
        }
        else
        {
            if (sin_time > 0f) sin_time = reset_wave_time(sin_time, sin_frequency, dt, sin_quadrant_on_key_released);
            if (cos_time > 0f) cos_time = reset_wave_time(cos_time, cos_frequency, dt, cos_quadrant_on_key_released);
            if (a_sin_time > 0f) a_sin_time = reset_wave_time(a_sin_time, angular_sin_frequency, dt, a_sin_quadrant_on_key_released);
        }
        ...
        sin_this_frame = Mathf.Sin(sin_multiplier * x_this_frame);
        cos_this_frame = Mathf.Cos(cos_multiplier * y_this_frame);
        a_sin_this_frame = Mathf.Sin(angular_sin_multiplier * a_this_frame);
